...
bk_err_t lvgl_event_send_data_handle(media_mailbox_msg_t *msg)
{
    ...
    lv_avi_command_t *cmd_info = (lv_avi_command_t *)msg->param;

    switch (cmd_info->cmd_type)
    {
    case LV_CMD_PLAY:
        LOGW("%s%d: avi name=%s\n", __FUNCTION__, __LINE__, (const char *)cmd_info->cmd_data);
        ...
        // open new resource
        ret = bk_avi_play_open((const char *)cmd_info->cmd_data, true, true);
        if (ret != BK_OK) {
            LOGE("%s bk_avi_play_open failed\r\n", __func__);
            break;;
        }
    ...
    default:
        LOGW("%s%d: unknow command %d\n", __FUNCTION__, __LINE__, cmd_info->cmd_type);
        break;
    }
    return ret;
}
