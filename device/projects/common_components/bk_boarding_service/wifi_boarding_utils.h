...
typedef enum
{
    ...
    BOARDING_OP_GET_DEVICE_ID = 60000,
    BOARDING_OP_MAX
} boarding_opcode_t;
...
typedef struct
{
    char *ssid_value;
    char *password_value;
    char *auth_token_value;
    char *auth_token_first_half;
    char *auth_token_second_half;
    char *agora_convoai_server_url;
    ble_boarding_op_cb_t cb;
    uint8_t boarding_notify[2];
    uint16_t ssid_length;
    uint16_t password_length;
    uint16_t auth_token_length;
    uint16_t agora_convoai_server_length;
} ble_boarding_info_t;
